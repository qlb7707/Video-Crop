...
void CmyfunDlg::OnBnClickedButton5()
{
	...
	if(!m_rects.empty()&&m_fixedMatting) 
		m_rects.pop_back();
	if(m_capture)
	{
		CString add;
		...
		int id2=m_singleFileName[0].ReverseFind('\\');
		string prefix = ws2s(m_singleFileName[0].GetBuffer(0)).substr(id2 + 1, id - id2 - 1);

		dir2frm=ws2s(m_singleFileName[0].Left(id).GetBuffer(0))+'\\'+prefix+"_"+ws2s(add.GetBuffer(0));//add.Format(_T("frame_%04d"),m_framePos);
		CreateDirectory(m_singleFileName[0].Left(id),NULL);
		int n=m_rects.size();
		for(int i=0;i<n;i++) 
		{
			CString rectNo;
			rectNo.Format(_T("_%d.png"),i);
